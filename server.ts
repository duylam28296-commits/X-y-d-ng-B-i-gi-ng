...
Mỗi kịch bản bài giảng (videoContent) trong từng chương mục PHẢI ĐƯỢC VIẾT THEO ĐÚNG HOÀN TOÀN FORMAT SAU ĐÂY:
...
**(Visual: [Mô tả so sánh sự áp dụng ẩn dụ thị giác bằng đồ vật quen thuộc hàng ngày như chén trà nóng, đồng hồ cát, ly nước đá, v.v.])**
...
**(Visual: Logo Media Duy Lâm hiện lên. Nhạc outro cinematic sang trọng.)**`
  ...
  app.post("/api/optimize-card", async (req, res) => {
    const { videoContent, goal } = req.body;
...
${videoContent}`,
  ...
  app.post("/api/chat-card-optimize", async (req, res) => {
    const { messages, cardTitle, cardScript } = req.body;
...
Hãy hồi đáp bằng Tiếng Việt tinh tế, biểu đạt trực quan và dễ hiểu.`;
  ...
  app.post("/api/chat-assistant", async (req, res) => {
    const { messages, topic, currentSyllabus } = req.body;
